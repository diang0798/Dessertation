...
def experiment3(num_epochs, training_loader, validation_loader, lr, cr_round, batch_size, norm):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(training_loader):
            ...
            if epoch >= 15:
                ...
                template = img1.data.cpu().numpy()
                template = template[0].squeeze(0)
                ...
                for j in range(batch_size):
                    ...
                    # product = np.mean((warp_img - warp_img.mean()) * (template - template.mean()))
                    # ncc_std = warp_img.std() * template.std()
                    ncc = loss_f.ncc(warp_img, template)
                    # if ncc_std == 0:
                    #     ncc = 0
                    # else:
                    #     ncc = product / ncc_std
        ...
        print("Training: Epoch [%d/%d],  Loss: %.4f" % (epoch + 1, num_epochs, epoch_loss))
    ...
    torch.save(linear_cnn.state_dict(), 'model/experiment3/round' + str(cr_round) + '.pkl')